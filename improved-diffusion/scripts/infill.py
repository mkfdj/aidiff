...
def main():
    ...
    model_embs = model_embs.to(xm.xla_device())()
    ...
    if args.modality in ['synth', 'pos']:
        ...
        if args.eval_task_ == 'infill':
            ...
            if args.eval_task_ == 'control_attribute':
                ...
                # langevin_fn_selected = partial(langevin_fn3, debug_lst, model_control, model3.to(xm.xla_device())(),
                #                                label_ids.expand(args.batch_size, -1), 0.1)
                # # langevin_fn_selected = partial(langevin_fn1, debug_lst, model_control, model3.to(xm.xla_device())(),
                # #                                label_ids.expand(args.batch_size, -1), 0.1)
            ...
            if args.eval_task_ == 'control_attribute_compose':
                ...
                #     'wikitext-103-raw-v1_101_wp_full_multi16_v2').to(xm.xla_device())()
                model_control = Classifier_Tree.from_pretrained(
                    'predictability/diff_models/e2e-tgt-tree_e=20_b=32_m=bert-base-uncased_'
                ...
                for target_length in control_length_lst:
                    ...
                    tgt_embs = model3.to(xm.xla_device())()(label_ids.to(xm.xla_device()))
            ...
            while len(all_images) * args.batch_size < args.num_samples:
                ...
                if args.eval_task_.startswith('control'):
                    ...
                    for sample in loop_func_(
                            ...
                            # denoised_fn=partial(langevin_early, model_control, model3.to(xm.xla_device())(),
                            #                     label_ids.expand(args.batch_size, -1), 0.1),
