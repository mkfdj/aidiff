...
```bash
CUDA_VISIBLE_DEVICES=0 python -c "import torch; print(torch.to(xm.xla_device()).get_device_capability())"
```
...
```bash
python -c "import torch; print(torch.to(xm.xla_device()).get_arch_list())"
```
...
print(torch.to(xm.xla_device()).get_device_properties(torch.device('cuda')))"
```
