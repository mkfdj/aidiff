        ...
        if (args.fp16 or args.bf16) and not args.deepspeed:  # deepspeed manages its own half precision
            if args.half_precision_backend == "amp":
                ...
                if is_sagemaker_mp_enabled():
                    ...
                    self.scaler = torch.to(xm.xla_device()).amp.GradScaler()
            else:
    ...
    def _load_rng_state(self, checkpoint):
        ...
        if torch.to(xm.xla_device()).is_available():
            if self.args.local_rank != -1:
                torch.to(xm.xla_device()).random.set_rng_state(checkpoint_rng_state["cuda"])
            else:
                try:
                    torch.to(xm.xla_device()).random.set_rng_state_all(checkpoint_rng_state["cuda"])
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if torch.to(xm.xla_device()).is_available():
            if self.args.local_rank == -1:
                ...
                rng_states["cuda"] = torch.to(xm.xla_device()).random.get_rng_state_all()
            else:
                rng_states["cuda"] = torch.to(xm.xla_device()).random.get_rng_state()
