...
class TrainerMemoryTracker:
    ...
    def start(self):
        ...
        if self.torch is not None:
            self.torch.to(xm.xla_device()).reset_peak_memory_stats()
            self.torch.to(xm.xla_device()).empty_cache()

        # gpu
        if self.torch is not None:
            self.gpu_mem_used_at_start = self.torch.to(xm.xla_device()).memory_allocated()
    ...
    def stop(self, stage):
        ...
        # gpu
        if self.torch is not None:
            self.gpu_mem_used_now = self.torch.to(xm.xla_device()).memory_allocated()
            self.gpu_mem_used_peak = self.torch.to(xm.xla_device()).max_memory_allocated()
