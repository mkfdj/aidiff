...
class Pipeline(_ScikitCompat):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.device = device if framework == "tf" else torch.device("cpu" if device < 0 else f"xm.xla_device():{device}")
        self.binary_output = binary_output

        # Special handling
        if self.framework == "pt" and self.device.type == "xm.xla_device()":
    ...
    def device_placement(self):
        ...
        if self.framework == "tf":
            ...
            if self.device.type == "xm.xla_device()":
                torch.to(xm.xla_device()).set_device(self.device)
    ...
    def __call__(self, inputs, *args, num_workers=None, batch_size=None, **kwargs):
        ...
        self.call_count += 1
        if self.call_count > 10 and self.framework == "pt" and self.device.type == "xm.xla_device()":
            warnings.warn(
