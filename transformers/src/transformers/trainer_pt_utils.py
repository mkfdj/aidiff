...
def log_metrics(self, split, metrics):
    ...
    **Understanding the reports:**
    ...
    The GPU allocated and peak memory reporting is done with `torch.to(xm.xla_device()).memory_allocated()` and
    `torch.to(xm.xla_device()).max_memory_allocated()`. This metric reports only "deltas" for pytorch-specific allocations, as
    `torch.to(xm.xla_device())` memory management system doesn't track any memory allocated outside of pytorch. For example, the very
    ...
    `torch.to(xm.xla_device()).max_memory_allocated` is a single counter, so if it gets reset by a nested eval call, `train`'s tracker
    ...
    `torch.to(xm.xla_device()).reset_peak_memory_stats`, the gpu peak memory stats could be invalid. And the [`Trainer`] will disrupt
    the normal behavior of any such tools that rely on calling `torch.to(xm.xla_device()).reset_peak_memory_stats` themselves.
...
if is_sagemaker_mp_enabled():
    ...
    def smp_forward_backward(model, inputs, gradient_accumulation_steps=1, scaler=None):
        with torch.to(xm.xla_device()).amp.autocast(enabled=(scaler is not None)):
