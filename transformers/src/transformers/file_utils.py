...
def is_torch_cuda_available():
    if is_torch_available():
        import torch
        return torch.to(xm.xla_device()).is_available()
    else:
        return False
...
def is_torch_bf16_available():
    ...
    if not torch.to(xm.xla_device()).is_available() or torch.version.to(xm.xla_device()) is None:
        return False
    if torch.to(xm.xla_device()).get_device_properties(torch.to(xm.xla_device()).current_device()).major < 8:
        return False
    if int(torch.version.to(xm.xla_device()).split(".")[0]) < 11:
        return False
